...
This document will be the single source of truth for the wire protocol between `WindowStreamServer`/`WindowStream.Cli` (.NET) and
`WindowStreamViewer` (Kotlin/Android XR). Both implementations reference this document.
...
Once implementation begins, this document takes over as the canonical reference, and the design spec becomes a historical record of the
decision process.
