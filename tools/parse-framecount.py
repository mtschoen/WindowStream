...
viewer = parse(sys.argv[1], {'reasm', 'dec', 'present'})
server = parse(sys.argv[2], {'convert', 'enc'})
...
print(f"skew estimate (server->viewer) = {skew} ms  (* = skew-corrected cross-device)")
...
for name, vals in rows:
    if not vals:
        print(f"{name:<34}{'--':>6}")
        continue
    print(f"{name:<34}{pct(vals,50):>6.0f}{pct(vals,95):>6.0f}{min(vals):>6.0f}{max(vals):>6.0f}{len(vals):>7}")
