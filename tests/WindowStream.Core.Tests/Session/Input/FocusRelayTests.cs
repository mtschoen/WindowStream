...
{
    sealed class FakeForegroundApi : IForegroundWindowApi
    {
        ...
        public uint GetWindowThreadProcessId(long hwnd) =>
            HandleToThread.TryGetValue(hwnd, out var thread) ? thread : 0;
    ...
    {
        var api = new FakeForegroundApi
        {
            Foreground = 0x100,
            HandleToThread = { [0x100] = 10, [0x200] = 20 }
        };
        var relay = new FocusRelay(api);

        var result = relay.BringToForeground(0x200);
    ...
    public void BringToForeground_NoOpIfAlreadyForeground()
    {
        ...
        var result = relay.BringToForeground(0x100);
